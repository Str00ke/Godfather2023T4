...
public class SimplePendulum : MonoBehaviour
{
    ...
    [SerializeField] float ropeLength = 2f;

    [SerializeField] Vector3 gravity = Vector3.one;
    ...
    Vector3 PendulumUpdate(Vector3 currentStatePosition, float deltaTime)
    {
        ...
        this.gravityForce = this.mass * gravity.magnitude;
        this.gravityDirection = gravity.normalized;
        ...
        if (distanceAfterGravity > this.ropeLength || Mathf.Approximately(distanceAfterGravity, this.ropeLength))
        {
            ...
            this.tensionForce = this.mass * gravity.magnitude * Mathf.Cos(Mathf.Deg2Rad * inclinationAngle);
